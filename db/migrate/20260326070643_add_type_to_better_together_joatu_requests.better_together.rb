...
# This migration comes from better_together (originally 20260312193000)
class AddTypeToBetterTogetherJoatuRequests < ActiveRecord::Migration[7.2]
  def up
    ...
    unless index_name_exists?(:better_together_joatu_requests, 'index_better_together_joatu_requests_on_type')
      add_index :better_together_joatu_requests, :type
    end
  end

  def down
    remove_index :better_together_joatu_requests, :type if index_name_exists?(:better_together_joatu_requests, 'index_better_together_joatu_requests_on_type')
