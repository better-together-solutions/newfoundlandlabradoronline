...
# This migration comes from better_together (originally 20260317221000)
...
class AddOauthClientSecretDigestToBetterTogetherPlatformConnections < ActiveRecord::Migration[7.2]
  def up
    add_column :better_together_platform_connections,
               :oauth_client_secret_digest, :string unless column_exists?(:better_together_platform_connections, :oauth_client_secret_digest)
  ...
  def down
    remove_column :better_together_platform_connections, :oauth_client_secret_digest if column_exists?(:better_together_platform_connections, :oauth_client_secret_digest)
  end
end
