...
# This migration comes from better_together (originally 20260315020100)
...
class CreateBetterTogetherOneTimePrekeys < ActiveRecord::Migration[7.2]
  def change
    ...
    add_index :better_together_one_time_prekeys, %i[person_id key_id], unique: true unless index_exists?(:better_together_one_time_prekeys, %i[person_id key_id])
    add_index :better_together_one_time_prekeys, %i[person_id consumed] unless index_exists?(:better_together_one_time_prekeys, %i[person_id consumed])
  end
end
