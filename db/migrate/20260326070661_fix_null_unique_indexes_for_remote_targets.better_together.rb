...
# This migration comes from better_together (originally 20260317000001)
...
class FixNullUniqueIndexesForRemoteTargets < ActiveRecord::Migration[7.2]
  def up
    ...
    # Remote grantee
    unless index_name_exists?(:better_together_person_access_grants, 'index_bt_person_access_grants_remote_grantee_unique')
      add_index :better_together_person_access_grants,
  ...
  def down
    remove_index :better_together_person_links, name: 'index_bt_person_links_local_target_unique' if index_name_exists?(:better_together_person_links, 'index_bt_person_links_local_target_unique')
    remove_index :better_together_person_links, name: 'index_bt_person_links_remote_target_unique' if index_name_exists?(:better_together_person_links, 'index_bt_person_links_remote_target_unique')
    ...
    remove_index :better_together_person_access_grants,
                 name: 'index_bt_person_access_grants_local_grantee_unique' if index_name_exists?(:better_together_person_access_grants, 'index_bt_person_access_grants_local_grantee_unique')
    remove_index :better_together_person_access_grants,
                 name: 'index_bt_person_access_grants_remote_grantee_unique' if index_name_exists?(:better_together_person_access_grants, 'index_bt_person_access_grants_remote_grantee_unique')
